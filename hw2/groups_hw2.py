...
class GroupElement:
    ...
    def left_lifted_action(self, vector):
        def left_action(v):
            group_element = self.group.element(v)
            return self.left(group_element).value
        
        diff = self._numerical_diff(left_action, vector)
        return diff

    def right_lifted_action(self, vector):
        def right_action(v):
            group_element = self.group.element(v)
            return self.right(group_element).value
        
        diff = self._numerical_diff(right_action, vector)
        return diff
    
    def Ad(self, vector):
        left_lift = self.left_lifted_action(vector)
        right_lift = self.right_lifted_action(vector)
